...
export function ThreadCard({
  ...
  return <div className={cn('relative flex flex-col items-center p-3 rounded-xl border-2 transition-all duration-300', 'backdrop-blur-md bg-card/40', 'min-w-[90px]', borderColorMap[professor.name], !professor.isAlive && 'opacity-40 grayscale', isRunning && ['scale-110 z-10', 'shadow-lg', glowColorMap[professor.name], 'ring-2 ring-primary/50'])}>
      {/* Indicador RUN */}
      {isRunning && professor.isAlive && <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-2 py-0.5 rounded-full bg-primary text-primary-foreground text-[10px] font-bold animate-pulse">
          ▶ RUN
        </div>}
      ...
      {/* Status Badge */}
      <span className={cn('mt-1 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase', status === 'RUN' && 'bg-primary/20 text-primary', status === 'READY' && 'bg-green-500/20 text-green-400', status === 'DEAD' && 'bg-destructive/20 text-destructive')}>
        {status}
      </span>
    </div>;
}